...
//* 텍스트 압축
//enable text compression: 서버로 부터 리소스를(js,css,html) 받을떄 텍스트를 압축하라는 뜻
//네트워크탭에서 api response header을 보면 content-encoding: gzip으로 되어있다. gzip으로 인코딩 방식을 통해 압축되어있다는 뜻(일반적으로 gzip 또는 deflate방식을 이용)
//반면에 번들링된 chunk파일의 경우 content-encoding 항목이 없는것을 확인 할  수 있다.
//압축은 번들파일을 서빙해주는 서버에서 해줘야함
//node ./node_modules/serve/bin/serve.js --help 명령어 치면, u옵션이 no compression이라는것을 확인할 수 있다
//따라서 serve스크립트 명령어인 npm run build && node ./node_modules/serve/bin/serve.js -u -s build -> npm run build && node ./node_modules/serve/bin/serve.js -s build
//파일크기가 2kb이상인 경우만 encoding 압축 해주는게 좋음
...
// reduce javascript excution time: 퍼포먼스탭 이용
//enable text compression: 서버로 부터 리소스를(js,css,html) 받을떄 텍스트를 압축하라는 뜻
